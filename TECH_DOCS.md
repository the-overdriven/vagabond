...
The **Crypt is a separate structure** and should not be treated as part of the
ordinary z:-1 → z:-2 → z:-3 cave chain.
...
same underground level**. The crypt is kept separate from the randomly
generated cave regions by world-connectivity validation.
...
On surce, the cemetery next to the ruined chapel (crypt entrance) contains graves.
...
Reserved for the Dwarven Fort (currently)

It is not generated as a normal random cave.

## z:-4: Crypt Level 2

The crypt has a second, dedicated level at **z:-4**.

It is not part of the shared z:-2 → z:-3 depth chain. It is reached from the
crypt using dedicated `cryptstairsdown` / `cryptstairsup` tiles.
